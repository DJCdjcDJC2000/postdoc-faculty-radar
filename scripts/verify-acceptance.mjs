...
check("core pages are present", ["page-home", "page-funding", "page-radar", "page-industry", "page-routes", "page-cases", "page-calendar", "page-methods"].every((id) => html.includes(id)));
...
check("funding intelligence and coverage are published", (publicSite.funding?.opportunities ?? []).length >= 40 && (publicSite.funding?.coverage ?? []).length >= 50);
check("funding action desk keeps private data browser-only", ["faculty-radar:funding-workspace:v1", "AES-GCM", "PBKDF2", "funding-deadlines.ics"].every((token) => app.includes(token)) && !JSON.stringify(publicSite.funding).includes("mentorContacts"));
